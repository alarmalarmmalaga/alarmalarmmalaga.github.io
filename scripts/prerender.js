...
async function prerender() {
  ...
  for (const lang of languages) {
    ...
    // Inject Canonical
    const canonicalTag = `<link rel="canonical" href="${baseUrl}/${langSuffix}" />`;
    homeHtml = homeHtml.replace('<!--CANONICAL_PLACEHOLDER-->', canonicalTag);
    ...
    for (const album of data.albums) {
      ...
      const albumCanonical = `<link rel="canonical" href="${baseUrl}/${langSuffix}albums/${slug}/" />`;
      albumHtml = albumHtml.replace('<!--CANONICAL_PLACEHOLDER-->', albumCanonical);
